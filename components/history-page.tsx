              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {courses
                      .reduce((total, course) => {
                        const hours = Number.parseFloat(course.timeSpent.split(" ")[0])
                        return total + hours
                      }, 0)
                      .toFixed(1)}
                    h
              ...
              <Card className="group cursor-pointer transition-all duration-200 hover:shadow-lg hover:scale-[1.02]">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className={`font-medium ${getGradeColor(course.grade)}`}>{course.grade}</span>
                  </div>

                  {/* Dates */}
                  <div className="text-xs text-muted-foreground">
                    <p>Started: {new Date(course.startDate).toLocaleDateString()}</p>
                    {course.completedDate && <p>Completed: {new Date(course.completedDate).toLocaleDateString()}</p>}
                  </div>
                </CardContent>
              </Card>
            </Link>
